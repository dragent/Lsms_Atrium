...
namespace App\Tests\Lsms;

use App\Tests\Factory\UserFactory;
use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;
use Symfony\Component\Routing\Generator\UrlGeneratorInterface;

class CareSheetTest extends WebTestCase
{
 /**
    ...
    public function testIsAnonymousRedirected(): void
    {
        ...
        $url= "https://127.0.0.1:8000".$urlGenerator->generate('app_lsms_care_sheet');
    ...
    public function testIsAdminConnected()
    {
        $client = self::createClient();
        $user = UserFactory::createOne();
        $user->object()->setRoles(['ROLE_LSMS']);
        $user->save();
        $client->loginUser($user->object());
        $urlGenerator = self::getContainer()->get(UrlGeneratorInterface::class);
        $url= "https://127.0.0.1:8000".$urlGenerator->generate('app_lsms_care_sheet');
