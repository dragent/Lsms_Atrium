...
class DiscordService {
     ...
     */
    private const DISCORD_ID ="1233049081236951221";
    ...
    const DISCORD_USER_ME_ENDPOINT ="/api/users/@me/guilds/".self::DISCORD_ID."/member";
    const LIST_DISCORD_USER = "https://discord.com/api/users/@me/guilds";
    const DISCORD_JOIN_USER = "/api/guilds/".self::DISCORD_ID."/members/";
    ...
    {
        $response= $this->discordApiClient->request(Request::METHOD_GET, self::DISCORD_USER_ME_ENDPOINT,[
            'auth_bearer'=>$accessToken,
        ]);
    ...
    public function userHasDiscordServ( string $accessToken  ): int|bool
    {
        ...
        foreach ($discord_list as $key => $value) {
            ...
            if ($discord_server['id']===self::DISCORD_ID)
                return true; 
        }
        return false;
    }
     ...
     */
    public function joinDiscord( string $accessToken , DiscordUser $discordUser)
    {
        $payload = [
